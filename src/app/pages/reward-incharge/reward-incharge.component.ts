...
@Component({
  selector: 'app-reward-incharge',
  templateUrl: './reward-incharge.component.html',
  styleUrls: ['./reward-incharge.component.scss']
})

export class RewardInchargeComponent {
  ...
  constructor(private commonService: CommonService, private brearcumbService: BreadcrumbService) {
    ...
    this.siteTable.push(
      { 'checked': true, 'item_no': 123, 'vehicle_no': 1234, 'material_desc': 'abcde', 'weight': 100, 'document_date': '23-10-2016' },
      { 'checked': false, 'item_no': 1234, 'vehicle_no': 12345, 'material_desc': 'abcde', 'weight': 120, 'document_date': '23-11-2016' },
      { 'checked': true, 'item_no': 12345, 'vehicle_no': 12345, 'material_desc': 'abcde', 'weight': 130, 'document_date': '23-12-2016' }
    )
  ...
  fillPurchaseForm() {
    ...
    // this.purchaseForm['controls']['bank_detail'].setValue(this.editPurchaseData['bankAccountNo']);
    ...
    // this.purchaseForm['controls']['entry_date'].setValue(moment(this.editPurchaseData['entryDate']).format("YYYY-MM-DD"));
    // this.purchaseForm['controls']['entry_date'].setValue(this.editPurchaseData['Entry Date']);
    // this.purchaseForm['controls']['invoice_date'].setValue(this.editPurchaseData['invoiceDate']);
    // this.purchaseForm['controls']['lr_number'].setValue(this.editPurchaseData['lrNumber']);
    // this.purchaseForm['controls']['status'].setValue(this.editPurchaseData['status']);
    ...
    // this.purchaseForm['controls']['entry_date'].disable();

    if (this.editPurchaseData.poCalculateItem.length > 0) {
      this.migoFormData = this.editPurchaseData.poCalculateItem.map((item: any) => {
        return {
          po_number: this.editPurchaseData.poNumber,
          po_item_no: item['purchaseOrderItemNo'],
          quantity: item['quantity']
        }
      })
    }
    this.migoForm.controls.po_number.setValue(this.editPurchaseData.poNumber)
    this.migoForm.controls.po_item_no.setValue(this.editPurchaseData.poCalculateItem[0].purchaseOrderItemNo);
    ...
    if (this.editPurchaseData.status == 'processing' || this.editPurchaseData.status == 'accept' || this.editPurchaseData.reviewerRemarks) {
      // this.purchaseForm['controls']['series_type'].setValue(this['editPurchaseData']['seriesType']);
      // this.purchaseForm['controls']['series_type'].disable();
      // this.purchaseForm['controls']['reviewer_remarks'].disable();
      ...
      this.commonService.dataGet(url).subscribe((res: any) => {
        ...
        if (res?.data.length > 0) {
          this.migoForm.controls.quantity.setValue(res.data[0].quantity);
          this.migoForm.controls.service_sheet_no.setValue(res.data[0].serviceSheetNo);
          this.migoForm.controls.quantity.disable();
          this.createMigo = true;
          this.migoFormData.forEach((item: any) => {
            item['grn'] = res.data[0].serviceSheetNo
          })
          if (this.editPurchaseData.status != 'accept') {
            this.selectGRNFromServiceSheetNo(res.data[0].serviceSheetNo);
          }
        }
      }, err => {
        console.log(err);
  ...
  getPODetail(po_number?: any) {
    ...
    this.commonService.dataGet(url).subscribe((res: any) => {
      ...
      if (res['data'] && res['data']['poItems'].length > 0) {
        ...
        this.getSubmissionTo(res['data']['poItems'][0]['plantCode'], this.editPurchaseData['invoiceType'], res['data']['poItems'][0]['preqNo']);
      }
  ...
  checkAll(event: any) {
    ...
    if (event.target.checked) {
      ...
      // this.selectedGRNArr = [...this.filterGrnDetails];
      ...
      this.purchaseForm['controls']['grn_arr
    }else{
      /* this.poGrnDetails.forEach((item:any)=>{
        item['checked'] = false;
      }) */
      ...
      this.filterGrnDetails.map((element: any) => {
        this.selectedGRNArr.map((item: any, i: any) => {
          if (item['refInvoiceNumber'] == element['refInvoiceNumber']) {
            // this.selectedGRNArr.splice(this.selectedGRNArr[i],1);
  ...
  verifyMigoAmount(event?: any, row?: any) {
    console.log('verifyMigoAmount');

    if (Number(event.target.value) == Number(row.quantity)) {
      // if(this.wrongInputArray.indexOf(row['po_item_no'])==-1){
      this.wrongInputArray.splice(this.wrongInputArray.indexOf(row['po_item_no']), 1);
      event.srcElement.nextSibling.style.display = 'none';
      // }
    } else {
      if (this.wrongInputArray.indexOf(row['po_item_no']) == -1) {
        this.wrongInputArray.push(row['po_item_no']);
    ...
    this.selectedItemsDataArr.map((item: any) => {
      q = q + Number(item['quantity'])
    })
    if (q == Number(event.target.value)) {
      this.migoForm['controls']['quantity'].setErrors();
      this.migoForm['controls']['quantity'].clearValidators();
    } else {
      this.migoForm['controls']['quantity'].setErrors({ 'quantity_mismatch': true });
    }
    /* let value = event.target.value * this.migoForm.value.gr_price;
    this.migoForm.controls.amount.setValue(value);
    if(value == this.totalGRNQuantity){
      this.migoForm.status = 'VALID';
    }else{
      this.migoForm.status = 'INVALID';
    }
    console.log(this.migoForm.valid); */
  }

  submitMigoForm() {
    console.log('submitMigoForm');
    ...
    let poList: any = []
    this.migoFormData.map((ele: any) => {
      poList.push(
        {
          "poNumber": ele['po_number'],
          "poItemNo": ele['po_item_no'],
    ...
    this.commonService.dataPost(url, json).subscribe((res: any) => {
      console.log(res);
      if (res?.status == 'Success' && res['data']['Records']?.MESSAGE) {
        this.commonService.spinner.hide();
        // this.commonService.routeToDashboard();
        this.editPurchaseData.status = 'processing';
        this.migoForm.controls.service_sheet_no.setValue(res['data']['Records']['MESSAGE'].split(':')[1].split('-')[0].trim());
        // this.selectGRNFromServiceSheetNo(this.migoForm.value.service_sheet_no);
        this.editPurchaseData.reviewerRemarks = this.purchaseForm.value.reviewer_remarks;
        this.getPODetail(this.editPurchaseData.poNumber);
        this.migoForm.controls.quantity.disable();
        this.createMigo = true;
        this.successToast = true;
        this.toastMsg = 'Migo created successfully';
        setTimeout(() => {
          this.successToast = false;
        }, 2000);
      } else {
      ...
      this.commonService.spinner.hide();
      console.log(err);
      this.toastMsg = err.error?.message?.split('EA')[0].trim() || 'Something went wrong';
      this.errorToast = true;
      setTimeout(() => {
        this.errorToast = false;
      }, 2000)
  ...
  selectGRNFromServiceSheetNo(service_sheet_no: any) {
    this.commonService.spinner.hide();
    this.apipoGrnDetails.map((item: any) => {
      if (item['materialDocumentNumber'] == service_sheet_no) {
        item['checked'] = true;
        item['status'] = 'done';
        item['updatedBy'] = this.username;
        item['updatedDate'] = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
